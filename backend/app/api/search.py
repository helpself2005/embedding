...
# mcp接口测试
@router.post("/search_image", operation_id="search_image", summary="搜索图片", 
    description=f"图片支持类型:{ALLOWED_EXTENSIONS}", response_model=SearchResponse)
async def search_image(files: SearchRequest,
    milvus_client:MilvusDB = Depends(get_milvus_client))-> SearchResponse:
    ...
    try:
        ...
        one_image_search_dto = OneImageSearchDTO(**{"file_data":file_data, "file_name":files.file_name, "file_type":file_type, "top_k":files.top_k, "cosine":files.cosine})    
        search_data = await asyncio.to_thread(search_image_service, one_image_search_dto, milvus_client)
  
        return SearchResponse(code=MessageCode.SUCCESS, msg=MessageStatus.SUCCESS, data=f"{search_data}")
...
# 网页图片搜索测试
@router.post("/api_search_image", operation_id="api_search_image", summary="网页图片搜索测试", 
    description=f"图片支持类型:{ALLOWED_EXTENSIONS}", response_model=SearchResponse)
async def api_search_image(files: List[UploadFile] = File(...),
    milvus_client:MilvusDB = Depends(get_milvus_client))-> SearchResponse:
    file_status = []
    tmp_dir = os.path.join(settings.mkdtempdir, "predict")
    if not os.path.exists(tmp_dir):
    ...
    try:
        for uploaded_file in files:
            ...
            if file_extension not in ALLOWED_EXTENSIONS:
                resdata = {"filename": uploaded_file.filename, "filestatus": "fail", "fileresult": []}
            ...
            async with aiofiles.open(tmp_file, 'wb') as out_file:
                await out_file.write(file_data)
            one_image_search_dto = OneImageSearchDTO(**{"file_data":file_data, "file_name":uploaded_file.filename, "file_type": uploaded_file.content_type, "top_k":5, "cosine":0.25})    
            search_data = await asyncio.to_thread(search_image_service, one_image_search_dto, milvus_client)
            # if vectorize_data.get("output", []):
            #     failed_files.append({"filename": uploaded_file.filename})
            file_status.append({"filename": uploaded_file.filename, "filestatus": "success", "fileresult": search_data})
